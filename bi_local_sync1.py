...
MONITOR_FOLDER_ID = "placeholder"
LOCK_FOLDER_ID = "placeholder"
...
def get_drive_service():
    creds = None
    if os.path.exists('placeholder.json'):
        creds = Credentials.from_authorized_user_file('placeholder.json', SCOPES)
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file('placeholder.json', SCOPES)
            creds = flow.run_local_server(port=0)
        with open('placeholder.json', 'w') as token:
    ...
    if int(setter) == 1:
        ...
        while True:
            ...
            if choice == '1':
                ...
                input_mode_id = input("Enter 1) MONITOR_FOLDER_ID, 2) placeholder, or 3) generate_instruction_set(), 4) generate_instruction_set_ELEMENT3(), or 5) generate_instruction_set_DESIGNATION4") or int("2")
